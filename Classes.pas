...
{                                                       }
{*******************************************************}
{  Minimalistic edition for non-VCL apps                }
{  Makes EXE about 140 kB smaller!                      }
{  All TComponent, TReader, TWriter etc stuff excluded  }
{  Define MiniRTL variable to switch mini-mode on       }
{  (and ensure the unit is included before              }
{  its standard RTL copy!)                              }
{  If you use RTL units that use Classes, you'll have   }
...
{$IFDEF MSWINDOWS}
uses Windows, Messages, SysUtils{$IFNDEF MiniRTL}, TypInfo, Variants, ActiveX{$ENDIF};
{$ENDIF}
{$IFDEF LINUX}
uses Libc, SysUtils, {$IFNDEF MiniRTL}Variants, TypInfo,{$ENDIF} Types;
{$ENDIF}
...
begin
  ...
  begin
    LCount := FCount;
    LList := FList;
    for Result := LCount - 1 downto 0 do
      if LList[Result] = Item then
        Exit;
    Result := -1;
  end;
